...
def certificate_generator():
    ...
    def task():
        if file_path != '':
            data_results_list = generate_certificates(file_path, preview_email_var.get())
            results = customtkinter.CTk()
            results.title('')
            results.iconbitmap(f'{os.getcwd()}\\static\\transparent.ico')
    ...
    description = customtkinter.CTkLabel(certificate_generator_frame, text='Generate certificates for completion of a training\n course and then automatically email the certificate\n to the delegate.',text_color='white', fg_color='transparent', font=custom_font)
    description.pack()
    ...
    preview_email_var = customtkinter.BooleanVar(value=True)
    preview_email = customtkinter.CTkCheckBox(certificate_generator_frame, variable=preview_email_var, text='Preview Emails',width=300, height=40, fg_color='#152F62', font=custom_font, text_color='white', border_color='white', border_width=2, onvalue=True, offvalue=False)
    preview_email.pack()

    select_data_button = customtkinter.CTkButton(certificate_generator_frame, text='Select Data', command=import_file, width=300, height=40, corner_radius=50, fg_color='#152F62', font=custom_font)
...
root.iconbitmap(f'{os.getcwd()}\\static\\transparent.ico')
